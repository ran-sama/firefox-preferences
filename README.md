...
## The Firefox 78 look is still possible with version 156+
...
4) This repo already provides the single file needed (licenses and credit included) from: [Aris-t2/CustomCSSforFx](https://github.com/Aris-t2/CustomCSSforFx/releases/tag/4.5.4) (FF156 works fine with v4.5.4)
...
#### 24. Ketuvia: accessible captions ([link](https://addons.mozilla.org/en-US/firefox/addon/ketuvia-accessible-captions/))
Replaces YouTube's word-by-word auto-captions with customizable phrase-based captions for better readability and accessibility.
...
## A case against pruning settings

```
user_pref("network.security.ports.banned.override", "0-65535");
https://searchfox.org/mozilla-central/search?q=network.security.ports.banned.override
https://searchfox.org/mozilla-esr60/source/dom/ipc/ContentPrefs.cpp#283
https://searchfox.org/firefox-esr115/source/netwerk/base/nsIOService.cpp#1470
```
Sometimes a setting looks as if gone from core code, but it can be hidden via strcmp splitting it up for no good reason, making the search return no result. In the NVME era you won't notice any delay on first start-up and losing a working setting would be worse than the imperceivable extra microsecond. Which is why it is often time wasted to track down if a setting was deliberately hidden and still is useful. Sometimes I will try to prune settings that are truly confirmed gone, like the failed Pocket API the userbase rejected over a whole decade.  
...
![alt text](https://raw.githubusercontent.com/ran-sama/firefox-preferences/master/images/essential_profile_DBs.png)
...
* logins.db - stores all your saved passwords (json on old versions)
* prefs.js - browser settings
* extension-preferences.json - addon activation state, permissions and settings
* extensions.json - addon activation state, permissions and settings
* extension-settings.json - addon activation state, permissions and settings
