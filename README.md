...
```
Sometimes a setting looks as if it's gone from central source, but it may as well just be hidden, due to the string being split now strcmp. In the NVME era you won't notice any delay on first start-up and losing a working setting would be worse than the imperceivable extra microsecond.  
